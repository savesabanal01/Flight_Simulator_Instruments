...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "MOBIFLIGHT_ThreeSFlightSimsI2C") == 0)
        _customType = MOBIFLIGHT_ThreeSFlightSimsI2C;

    if (_customType == MOBIFLIGHT_ThreeSFlightSimsI2C) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(ThreeSFlightSimsI2C))) {
        ...
        ********************************************************************************** */
        ...
        _myThreeSFlightSimsI2C = new (allocateMemory(sizeof(ThreeSFlightSimsI2C))) ThreeSFlightSimsI2C(_addrI2C);
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == MOBIFLIGHT_ThreeSFlightSimsI2C) {
        _myThreeSFlightSimsI2C->set(messageID, setPoint);
    }
}
